...
parser.add_argument('-dynamic_threshold', type=float, default=0.4)
...
class VideoApp:
    def __init__(self, window, window_title, ssd_location, model_location, location, location_number, threshold, seconds, percent, deviation_seconds, deviation_stddev, dynamic_seconds, dynamic_threshold, dynamic_stddev, no_email, ltch_wifi, wemo_wifi, from_email, to_emails):
        self.window = window
        ...
        while self.running:
            with (self.lock):
                if len(self.indices) == 0 or (len(self.indices) > 0 and self.index != self.indices[-1]): 
                    ...
                    elif not self.pain_moment and not self.baseline and self.count[-1] - self.count[0] >= self.MAX_FRAMES - 1 and self.count[0] >= self.end_index:
                        for c in self.count:
                            try:
                                i = self.indices.index(c)
                                i_ = self.count.index(c)
