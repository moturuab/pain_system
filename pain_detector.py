...
class PainDetector:
    def __init__(self, num_outputs, checkpoint_path='', fan_checkpoint='', image_size=160):
        ...
        # use YOLO to check if there is a face
    ...
    def add_references(self, image_list):
        ...
        for image in image_list:
            im = self.prep_image(image)
            if im is not None:
                self.ref_frames.append(im)
    ...
    def prep_image(self, image, scale_to=320):
        ...
        if landmarks is None:
            return None
        if len(landmarks) > 1:
            ValueError('Reference image had more than one face. It should only have one')
        else:
        ...
        return torch.from_numpy(image_face).to(self.device) / 255
    ...
    def predict_pain(self, image):
        ...
        with torch.no_grad():
            ...
            for ref_frame in self.ref_frames:
                frames = torch.cat([target_frame, ref_frame], dim=1).to(self.device)
